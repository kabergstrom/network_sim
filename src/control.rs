...
impl<'s> amethyst::ecs::System<'s> for GuiSystem {
    ...
    fn run(&mut self, (_screen_dimensions, time, sim, mut settings): Self::SystemData) {
        ...
        if settings.playing {
            ...
            settings.curr_time = settings.curr_time % settings.seconds;
        }
        amethyst_imgui::with(|ui| {
            use amethyst_imgui::imgui::*;
            Window::new(im_str!("control")).build(ui, || {
                ...
                let mut changed =
                    Slider::new(im_str!("server fps"), 1..=240).build(ui, &mut settings.server_fps);
                changed |=
                    Slider::new(im_str!("client fps"), 1..=240).build(ui, &mut settings.client_fps);
                changed |=
                    Slider::new(im_str!("sync rate"), 1..=240).build(ui, &mut settings.sync_rate);
                ...
                changed |=
                    Slider::new(im_str!("seconds"), 0.1..=5.0).build(ui, &mut settings.seconds);
                changed |= ui.small_button(im_str!("Sim"));
